...
DEFAULT_CONFIG = {
    ...
    "security": {
        ...
        "allow_command_separators": True,  # Allow pipes, semicolons, ampersands
    },
    "commands": {
        "read": [
            ...
            if config_key.startswith("commands_"):
                ...
                if setting in self.config["security"]:
                    ...
                    if isinstance(default_value, bool):
                        self.config["security"][setting] = value.lower() in ["true", "1", "yes"]
                ...
                if setting in self.config["server"]:
                    ...
                    if isinstance(default_value, bool):
                        self.config["server"][setting] = value.lower() in ["true", "1", "yes"]
                    elif isinstance(default_value, int):
                        try:
                ...
                if setting in self.config["output"]:
                    ...
                    if isinstance(default_value, bool):
                        self.config["output"][setting] = value.lower() in ["true", "1", "yes"]
                    elif isinstance(default_value, int):
                        try:
