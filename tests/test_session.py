...
def test_get_session_existing():
    ...
    session_manager.sessions[session_id]["last_active"] = original_last_active - 1.0
    ...
    assert updated_session["last_active"] > session_manager.sessions[session_id]["last_active"] - 0.5
...
def test_has_command_type_approval():
    ...
    assert not session_manager.has_command_type_approval(session_id, command_type)
