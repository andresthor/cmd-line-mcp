...
async def test_output_truncation_simulation(server):
    ...
    truncated_output = long_output[:truncated_size] + "\n... [output truncated due to size]"
    
    # Assertions without mocking the actual method
    assert len(truncated_output) <= truncated_size + 50  # Allow for truncation message
...
async def test_command_validation_mismatch(server):
    ...
    try:
        ...
        result = await server._execute_command("test_command", command_type="read")
...
async def test_auto_approval_behavior(server):
    ...
    try:
        ...
        server.config.config["security"]["require_session_id"] = False  # Auto-approve for Claude Desktop
        ...
        with patch("cmd_line_mcp.security.validate_command", return_value=validation_result):
            with patch("asyncio.create_subprocess_shell") as mock_subprocess:
                ...
                result = await server._execute_command("test_command", session_id=session_id)
